...
try:
    ...
    dt_model = knn_model = svm_model = scaler = None
...
if page == "Home":
    ...
    st.write(
        """
This system predicts a person's obesity level using Machine Learning.
    ...
    if st.button("🔍 Predict Obesity Level", use_container_width=True):
        ...
        if active_model is None:
            ...
            if hasattr(active_model, "feature_names_in_"):
                aligned_input = input_data[active_model.feature_names_in_]
            elif scaler is not None and hasattr(scaler, "feature_names_in_"):
                aligned_input = input_data[scaler.feature_names_in_]
            else:
            ...
            st.write("Model:", active_model)
            st.write("Model classes:", active_model.classes_)
            st.write("Model feature names:", active_model.feature_names_in_)
            st.write("Input feature names:", aligned_input.columns.tolist())
            
            prediction_raw = active_model.predict(model_input)[0]
            
            st.write("DEBUG - Raw Prediction:", prediction_raw)
            st.write("DEBUG - BMI:", bmi)
            ...
            if isinstance(prediction_raw, (str, np.str_)):
                ...
                prediction_label = standard_alphabetical_map.get(
...
# ======================================
    ...
    if df_raw is None:
        ...
        with t1:
          ...
          # Chart 3: 100% Stacked Bar Chart with Percentage Labels (完美复刻目标第一图样式)
            ...
            # Chart 4: Violin Plot
            st.subheader(
                "4. Impact of Family History of Obesity on BMI (Violin Plot)"
            )
            fig, ax = plt.subplots(figsize=(9, 5))
            sns.violinplot(
                data=df,
        ...
        with t2:
           # Chart 5: Grouped Boxplot 
            ...
            # Chart 6: Point Plot
            st.subheader(
                "6. Interaction Between High-Calorie Food Consumption (FAVC) and Number of Main Meals (NCP) on BMI (Point Plot)"
            )
            df_chart6 = df.copy()
            ...
            ax.set_title(
                "Interaction Effect: Main Meals (NCP) & High-Calorie Food (FAVC) on BMI",
                fontsize=11,
                fontweight="bold",
                loc="left",
            )
            ...
            ax.legend(
                title="Frequent High-Calorie Food (FAVC)",
           ...
           # Chart 7: Crosstab Heatmap 
            ...
            # Chart 8: KDE Plot for Key Categories 
            ...
            sns.move_legend(ax, "upper left", bbox_to_anchor=(1.02, 1), title="Obesity Category", frameon=True)
           ...
           # Chart 9: Barplot with 95% CI & Mean Annotations
            st.subheader("9. Alcohol Consumption Frequency (CALC) vs Mean BMI (with 95% CI)")
            fig, ax = plt.subplots(figsize=(9, 5.5))

            df_chart9 = df.copy()
            if 'BMI' not in df_chart9.columns:
                df_chart9['BMI'] = df_chart9['Weight'] / (df_chart9['Height'] ** 2)
            ...
            means = df_chart9.groupby('CALC')['BMI'].mean().reindex(calc_order)
        ...
        # -----------------------------
        with t3:
            # Chart 10: FacetGrid Boxplot
            st.subheader(
                "10. Physical Activity Frequency (FAF) vs. Time Spent Using Technology Devices (TUE)"
            )
            df_chart10 = df.copy()
            ...
            g.fig.suptitle(
                "BMI Distribution across Physical Activity (FAF) and Tech Use (TUE) by Gender",
                fontsize=12,
                fontweight="bold",
            )
            ...
            # Chart 11: FacetGrid Boxplot (完美复刻第一个图样式)
            st.subheader(
                "11. Effect of Physical Activity Frequency (FAF) on BMI by Family History of Obesity"
            )
            df_chart11 = df.copy()
            ...
            g.fig.subplots_adjust(top=0.82)
            g.fig.suptitle(
                "Physical Activity (FAF) vs BMI across Family History of Overweight",
                fontsize=12,
                fontweight="bold",
            )
            g.set_axis_labels("Physical Activity Frequency (FAF: Days/Week)", "BMI ($kg/m^2$)")
      ...
      # Chart 14: Transportation Method Disparities between Extreme Weight Groups
        ...
        with t4:
           ...
           # Chart 18: Cumulative Risk Score vs. Mean BMI 
            ...
            ax.set_rlabel_position(35)  # 刻度数字摆放角度
...
# ======================================
    ...
    st.caption("Obesity Risk Prediction System & Data Analysis")
    st.markdown("---")

    st.info(
        "💡 **Project Overview:** "
        "This interactive Streamlit application predicts obesity risk levels based on individual "
        "demographics, eating habits, and daily physical activities using Machine Learning models."
    )
    ...
    with col1:
        ...
        st.markdown(
            """
        * **Predict Risk Levels:** Classify obesity status into 7 categories.
        * **Interactive EDA:** Visualize eating habits and physical activity correlations.
        * **Smart Insights:** Provide real-time predictions based on personal health metrics.
        """
        )
    ...
    with col2:
        ...
        * **Machine Learning:** Scikit-Learn (KNN, Decision Tree, Naive Bayes)
        """
        )

    st.markdown("---")

    with st.expander("📊 About the Dataset & Key Features"):
        st.markdown(
            """
        The model is trained on data incorporating various life factors:
        * **Demographics:** Gender, Age, Height, Weight, Family History with Overweight.
        * **Dietary Habits:** High-Calorie Food Intake (FAVC), Vegetable Intake (FCVC), Main Meals (NCP), Water Intake (CH2O), Alcohol Intake (CALC).
        * **Physical Activity & Lifestyle:** Physical Activity Frequency (FAF), Screen Time (TUE), Transport Mode (MTRANS), Calories Monitoring (SCC).
        """
        )

    st.markdown("<br><br>", unsafe_allow_html=True)
    st.caption("Developed with ❤️ for Health Analytics & Machine Learning Project.")